...
#include "src/SystemConf.h"
#include "src/input.h"    //inputo klase
#include "src/soundfx.h"
#include "src/oggplay.h"
#include "src/Picture3D.h"
#include "src/CClient.h"
#include "src/CServer.h"


#include "src/dude.h"
#include "src/map.h"
#include "src/maplist.h"
#include "src/BulletContainer.h"
#include "src/SelectMenu.h"
#include "src/EditBox.h"
#include "src/ScroollControl.h"
#include "src/WriteText.h"
...
void MoveDude(){
	...
	if ((Keys[0])||(Keys[1])||(Keys[2])||(Keys[3])){
		...
		if (isServer)
			mapas.mons[mapas.enemyCount].move(speed,sspeed,8.0f,mapas.colide, mapas.width, mapas.height, mapas.mons,mapas.enemyCount+serveris.clientCount()+1,&dirx,&diry);
		else
...
//------------------------------------------------
//monstru rases, itemu isidestymas
	...
	for (unsigned long i=0;i<mapas.items.count();i++){
		if (CirclesColide(mapas.mons[mapas.enemyCount].x,mapas.mons[mapas.enemyCount].y,8,
			mapas.items[i].x,mapas.items[i].y,4)){
				...
				if ((item!=0)&&((item!=6)||(mapas.mons[mapas.enemyCount].hp<100))){ //daiktas bus paimtas
				   ...
				   int ncmdshow)
{
	...
	windowsclass.hIcon = LoadIcon(hinstance,MAKEINTRESOURCE(102));
	...
	windowsclass.hIconSm = LoadIcon(hinstance,MAKEINTRESOURCE(102));
	...
	if (!(hwndMain = CreateWindowEx(
		NULL, "WINDOWSCLASS", "Warez D00ds 3",
  ...
  if (SUCCEEDED(InitD3D())){
			...
			while(msg.message != WM_QUIT){
				...
				else
				{
					...
					if ((timeGetTime()>tick)){ 
						...
						if (sys.shutdown)
							QuitApp();   
						tick=timeGetTime()+10;//kazkur apie ?fps
	  ...
	  MessageBox(hwndMain,"Video init failed!","Error",0);
